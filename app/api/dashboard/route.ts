// ============================================================================
// app/api/dashboard/route.ts (CORREGIDO)
// ============================================================================
...
export async function GET(req: NextRequest) {
  try {
    ...
    const cumplimientoGlobal  = clientesAsignados > 0
      ? Math.round((visitasTotales / clientesAsignados) * 100) : 0;

    // ── Por zona ─────────────────────────────────────────────────────────
    const zonasMap: Record<string, {
      zona: string; asesores: number; visitas: number; clientes: number; cumplimiento: number;
    }> = {};

    for (const a of equipoRows) {
      const z = a.zona || 'Sin zona';
      if (!zonasMap[z]) zonasMap[z] = { zona: z, asesores: 0, visitas: 0, clientes: 0, cumplimiento: 0 };
      zonasMap[z].asesores++;
