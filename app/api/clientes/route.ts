...
// PATCH → actualizar coordenadas GPS de un cliente existente
// ============================================================================
...
// ── Actualizar GPS de cliente existente ──────────────────────────────────────
export async function PATCH(req: NextRequest) {
  try {
    const body = await req.json();
    const { cliente_id, lat, lng } = body;

    if (!cliente_id || !lat || !lng) {
      return NextResponse.json(
        { error: 'cliente_id, lat y lng son requeridos' },
        { status: 400 }
      );
    }
