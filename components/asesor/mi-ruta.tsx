...
// ✅ Asesor puede ELIMINAR cliente duplicado directamente
        ...
        async (pos) => {
          try {
            await fetch('/api/ubicacion-asesor', {
              ...
              body: JSON.stringify({ asesor_id: ASESOR_ID, lat: pos.coords.latitude, lng: pos.coords.longitude }),
            })
          } catch {}
        },
        () => {},
  ...
  if (vista === "gestion" && clienteActivo) {
    return (
      <GestionCliente
        ...
        onEliminado={() => { setVista("lista"); setClienteActivo(null); mutate() }}
      />
    )
  }
...
function GestionCliente({ cliente, asesorId, userLocation, isOnline, onVolver, onExito, onEliminado }: GestionClienteProps) {
  ...
  const handleEliminarCliente = async () => {
    if (!confirmEliminar) { setErrorEliminar("Marca la casilla de confirmación para continuar"); return }
    setEliminando(true)
    setErrorEliminar("")
    try {
      const res = await fetch('/api/clientes/eliminar', {
        ...
        body: JSON.stringify({
          cliente_id: cliente.id,
          asesor_id:  asesorId,
          motivo:     motivoEliminar.trim() || 'Duplicado eliminado por asesor',
        }),
      })
      const data = await res.json()
      if (!res.ok) {
        setErrorEliminar(data.error || "Error eliminando cliente")
        return
      }
      // Volver a la lista y refrescar
      onEliminado()
    } catch {
      setErrorEliminar("Error de conexión. Intenta nuevamente.")
    } finally {
      setEliminando(false)
    }
  }
    ...
    try {
      ...
      if (navigator.onLine) {
        try {
          ...
          clearTimeout(timer)
          if (res.ok) await eliminarVisitaOffline(offlineId)
        } catch {}
      }
    ...
    <div className="flex flex-col min-h-screen bg-dark-bg">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 pb-32">
        ...
        <div className="rounded-xl bg-dark-surface border border-white/10 p-4 space-y-2">
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-gray-300 flex-1">{dirMostrada || "Sin dirección"}</p>
            <button
              onClick={() => { setMostrarEditarCliente(!mostrarEditarCliente); setMostrarActualizarGPS(false); setMostrarEliminar(false) }}
          ...
          <div className="flex items-center gap-3 pt-1 border-t border-white/5 flex-wrap">
            <button
              onClick={() => { setMostrarActualizarGPS(!mostrarActualizarGPS); setMostrarEditarCliente(false); setMostrarEliminar(false) }}
            ...
            <span className="text-gray-700">·</span>
            <button
              onClick={() => { setMostrarEliminar(!mostrarEliminar); setMostrarActualizarGPS(false); setMostrarEditarCliente(false) }}
        ...
        {mostrarEditarCliente && !clienteGuardado && (
          <div className="rounded-xl border border-navy-accent/30 bg-navy-accent/5 p-4 space-y-3">
            ...
            <div>
              ...
              <input type="text" value={editDireccion} onChange={e => setEditDireccion(e.target.value)} placeholder="Ej: Calle 5 #10-20 Local 3" className="w-full rounded-xl border border-white/10 bg-dark-surface px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:border-navy-accent focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">Teléfono</label>
              <input type="tel" value={editTelefono} onChange={e => setEditTelefono(e.target.value)} placeholder="Ej: 3001234567" inputMode="tel" className="w-full rounded-xl border border-white/10 bg-dark-surface px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:border-navy-accent focus:outline-none" />
            </div>
            <button onClick={handleGuardarCliente} disabled={guardandoCliente || !editNombre.trim()}
              ...
              {guardandoCliente ? <><Loader2 className="h-4 w-4 animate-spin" /><span>Guardando...</span></> : <><Check className="h-4 w-4" /><span>Guardar cambios</span></>}
            </button>
          </div>
        )}
        ...
        {mostrarEliminar && (
          <div className="rounded-xl border border-danger/40 bg-danger/5 p-4 space-y-3">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm font-semibold text-white">Eliminar cliente duplicado</p>
                <p className="text-xs text-gray-400 mt-0.5">
                  Solo puedes eliminar clientes sin visitas registradas hoy. Esta acción no se puede deshacer.
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Ej: Este cliente aparece duplicado como 'TIENDA DON PEDRO' en ruta 3B..."
                rows={2}
                className="w-full resize-none rounded-xl border border-white/10 bg-dark-surface px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-danger/50 focus:outline-none"
              />
            </div>

            {/* Confirmación explícita */}
            <label className={`flex items-start gap-3 cursor-pointer rounded-xl border p-3 transition-all ${
              confirmEliminar ? 'border-danger/50 bg-danger/10' : 'border-white/10 bg-dark-surface'
            }`}>
              <div className="mt-0.5">
                <input
                  type="checkbox"
                  checked={confirmEliminar}
                  onChange={e => { setConfirmEliminar(e.target.checked); setErrorEliminar("") }}
                  className="h-4 w-4 rounded border-white/20 accent-red-500"
                />
              </div>
              <p className="text-xs text-gray-300 leading-relaxed">
                Entiendo que estoy eliminando a{" "}
                <span className="text-white font-semibold">{getNombreSinRuta(nombreMostrado)}</span>
                {" "}de mi cartera. Esta acción no se puede deshacer.
              </p>
            </label>
            ...
            {errorEliminar && (
              <div className="flex items-center gap-2 rounded-xl bg-danger/10 border border-danger/20 px-3 py-2">
                <AlertTriangle className="h-3.5 w-3.5 text-danger shrink-0" />
                <p className="text-xs text-danger">{errorEliminar}</p>
              </div>
            )}

            <div className="flex gap-2">
              <button
                onClick={() => { setMostrarEliminar(false); setConfirmEliminar(false); setMotivoEliminar(""); setErrorEliminar("") }}
                className="flex-1 rounded-xl border border-white/10 bg-dark-surface py-2.5 text-sm text-gray-400 hover:text-white transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-danger/20 border border-danger/40 py-2.5 text-sm font-semibold text-danger transition-all active:scale-[0.97] disabled:opacity-40"
              >
                {eliminando
        ...
        {(sinGPS || mostrarActualizarGPS) && !gpsGuardado && (
          <div className={`rounded-xl border p-4 ${sinGPS ? 'border-warning/40 bg-warning/10' : 'border-navy-accent/30 bg-navy-accent/5'}`}>
            <div className="flex items-start gap-3 mb-3">
              ...
              <div>
                <p className="text-sm font-semibold text-white">{sinGPS ? 'Sin ubicación GPS' : 'Actualizar ubicación'}</p>
                <p className="text-xs text-gray-400 mt-0.5">{sinGPS ? 'Párate en el punto exacto del cliente y captura.' : 'Párate en el punto exacto del cliente y actualiza las coordenadas.'}</p>
            ...
            <button onClick={handleCapturarGPS} disabled={guardandoGPS || !userLocation}
              ...
              {guardandoGPS ? <><Loader2 className="h-4 w-4 animate-spin" /><span>Guardando...</span></> : <><Navigation className="h-4 w-4" /><span>{sinGPS ? 'Capturar ubicación' : 'Guardar nueva ubicación'}</span></>}
            </button>
          </div>
        )}
        ...
        <div className={`rounded-xl border p-4 ${
          ...
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-full ${
              ...
              {sinGpsDispositivo ? <AlertTriangle className="h-5 w-5 text-danger" /> :
               gpsGuardado ? <Check className="h-5 w-5 text-success" /> :
               (!sinGPS && dentroDelRango) ? <Check className="h-5 w-5 text-success" /> :
               distancia !== null ? <AlertTriangle className="h-5 w-5 text-warning" /> :
            ...
            <div className="flex-1">
              {sinGpsDispositivo ? (
                <>
                  ...
                  <p className="text-xs text-gray-300 mt-0.5">📸 <span className="font-semibold text-white">Foto obligatoria</span> para poder registrar</p>
                </>
              ) : gpsGuardado ? (
        ...
        {yaVisitado ? (
          <div className="rounded-xl bg-dark-surface border border-white/10 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setMostrarEditarVisita(!mostrarEditarVisita)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-navy-accent transition-colors">
            ...
            {mostrarEditarVisita && (
              <div className="rounded-xl border border-navy-accent/30 bg-navy-accent/5 p-4 space-y-3 mt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => { setEditHuboPedido(false); setEditMonto("") }}
                    className={`flex flex-col items-center gap-1 rounded-xl border p-3 transition-all ${!editHuboPedido ? 'border-navy-accent bg-navy-accent/20 text-white' : 'border-white/10 bg-dark-surface text-gray-400'}`}>
                    <Eye className={`h-5 w-5 ${!editHuboPedido ? 'text-navy-accent' : 'text-gray-500'}`} />
                    <span className="text-xs font-semibold">Solo Visita</span>
                  </button>
                  <button onClick={() => setEditHuboPedido(true)}
                    className={`flex flex-col items-center gap-1 rounded-xl border p-3 transition-all ${editHuboPedido ? 'border-success bg-success/20 text-white' : 'border-white/10 bg-dark-surface text-gray-400'}`}>
                ...
                {editHuboPedido && (
                  <div className="rounded-xl bg-dark-surface border border-success/30 p-3">
                    <div className="flex items-center gap-2">
                      ...
                      <input type="number" value={editMonto} onChange={e => setEditMonto(e.target.value)} placeholder="0" min="0" step="1000" inputMode="numeric" autoFocus className="flex-1 bg-transparent text-2xl font-bold text-white placeholder-gray-600 focus:outline-none" />
          ...
          <>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setTipoGestion(tipoGestion === "visita" ? null : "visita")}
                  className={`flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${tipoGestion === "visita" ? "border-navy-accent bg-navy-accent/20 text-white" : "border-white/10 bg-dark-surface text-gray-400"}`}>
                ...
                <button onClick={() => setTipoGestion(tipoGestion === "pedido" ? null : "pedido")}
                  className={`flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${tipoGestion === "pedido" ? "border-success bg-success/20 text-white" : "border-white/10 bg-dark-surface text-gray-400"}`}>
            ...
            {tipoGestion === "pedido" && (
              <div className="rounded-xl bg-dark-surface border border-success/30 p-4">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-lg font-bold text-gray-400">$</span>
                  <input type="number" value={monto} onChange={e => setMonto(e.target.value)} placeholder="0" min="0" step="1000" autoFocus inputMode="numeric" className="flex-1 bg-transparent text-2xl font-bold text-white placeholder-gray-600 focus:outline-none" />
                  <span className="text-sm text-gray-500">COP</span>
                </div>
                {monto && parseFloat(monto) > 0 && <p className="mt-1 text-xs text-success">${parseFloat(monto).toLocaleString('es-CO')}</p>}
              </div>
            )}
            ...
            {tipoGestion && (
              <div>
                ...
                <textarea value={nota} onChange={e => setNota(e.target.value)} placeholder="Ej: Cliente no estaba, volver mañana..." rows={2}
            ...
            {tipoGestion && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  {fotoObligatoria && <span className="rounded-full bg-danger/20 px-2 py-0.5 text-[10px] font-bold text-danger">OBLIGATORIA</span>}
                </div>
                {!fotoPreview ? (
                  <button onClick={handleAbrirCamara} disabled={subiendoFoto}
                    className={`flex w-full items-center justify-center gap-3 rounded-xl border-2 border-dashed py-6 text-gray-400 transition-all active:scale-[0.98] disabled:opacity-50 ${
                      fotoObligatoria ? "border-danger/40 bg-danger/5 hover:border-danger/60" : "border-white/20 bg-dark-surface hover:border-navy-accent/50 hover:text-gray-300"
                    }`}>
                    {subiendoFoto ? <><Loader2 className="h-6 w-6 animate-spin" /><span className="text-sm">Procesando...</span></> : <><Camera className={`h-6 w-6 ${fotoObligatoria ? 'text-danger' : ''}`} /><span className="text-sm font-medium">{fotoObligatoria ? 'Tomar foto (requerida)' : 'Tomar foto'}</span></>}
                  </button>
                ) : (
                  <div className="rounded-xl overflow-hidden border border-success/30">
                    ...
                    <div className="flex items-center justify-between px-3 py-2 bg-black/60">
                      ...
                      <button onClick={() => { setFotoPreview(null); setFotoBase64(null) }} className="flex items-center gap-1 rounded-lg bg-white/10 px-2 py-1 text-xs text-white">
                ...
                <p className="text-[10px] text-gray-600 text-center">
                  {fotoPreview ? "La foto se subirá al confirmar" : fotoObligatoria ? "Requerida cuando el GPS del dispositivo está inactivo" : "Opcional — evidencia de la visita"}
                </p>
              </div>
            )}
          </>
      ...
      {!yaVisitado && tipoGestion && (
        <div className="fixed inset-x-0 bottom-16 z-40 px-4 pb-3">
          ...
          {sinGpsDispositivo && !fotoBase64 && <p className="mt-1.5 text-center text-[10px] text-danger font-semibold">📸 Toma la foto para poder confirmar sin GPS</p>}
          {sinGpsDispositivo && fotoBase64 && <p className="mt-1.5 text-center text-[10px] text-warning">SIN GPS — se registrará con foto como evidencia · marcado para supervisión</p>}
          {!isOnline && <p className="mt-1 text-center text-[10px] text-warning">SIN CONEXIÓN — se guardará y sincronizará al reconectar</p>}
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col min-h-screen bg-dark-bg">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 pb-32">
        ...
        <div>
          <label className="block text-xs text-gray-400 mb-1">Dirección</label>
          <input type="text" value={direccion} onChange={e => setDireccion(e.target.value)} placeholder="Ej: Calle 5 #10-20"
        ...
        <div className={`rounded-xl border p-4 ${usarGPS && userLocation ? "border-success/30 bg-success/10" : "border-white/10 bg-dark-surface"}`}>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={usarGPS} onChange={e => setUsarGPS(e.target.checked)} className="h-4 w-4 rounded border-white/20 text-success focus:ring-success" />
            <div>
              <p className="text-sm font-medium text-white">Capturar ubicación GPS ahora</p>
              <p className="text-xs text-gray-400">{usarGPS ? userLocation ? `📍 ${userLocation.lat.toFixed(5)}, ${userLocation.lng.toFixed(5)}` : "Obteniendo GPS..." : "Sin ubicación"}</p>
            </div>
          </label>
        </div>
        {error && (
      ...
      <div className="fixed inset-x-0 bottom-16 z-40 px-4 pb-3">
        <button onClick={handleCrear} disabled={loading || !nombre.trim()}
          ...
          {loading ? <><Loader2 className="h-5 w-5 animate-spin" /><span>CREANDO...</span></> : <><UserPlus className="h-5 w-5" /><span>CREAR CLIENTE</span></>}
        </button>
      </div>
    </div>
  )
